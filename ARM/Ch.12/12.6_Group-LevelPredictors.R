...
cty <- read.table("ARM/Ch.12/cty.dat", header = T, sep = ",")
...
radon_group.sf1 <- stan(
  file = 'ARM/Ch.12/radon_group.stan',
  data = dataList.3,
  iter = 500,
  chains = 4,
...
print(radon_group.sf1, pars = c("alpha", "beta", "sigma", "lp__"))
...
radon_no_pool.sf1 <-
  stan(
    file = 'ARM/Ch.12/radon_no_pool.stan',
    data = dataList.4,
    iter = 100,
    chains = 4
  )
...
post2.ranef <- colMeans(post2$a)
mean2.ranef <- mean(post2.ranef)
post2.fixef1 <- mean(post2$beta)
...
a.hat.M2 <-
  post1.fixef1 + post1.beta[2] * u + post1.ranef - mean1.ranef
b.hat.M2 <- post1.beta[1]

x.jitter <- x + runif(n, -.05, .05)
display8 <-
  c(36, 1, 35, 21, 14, 71, 61, 70)  # counties to be displayed
...
radon8.data$m1.int <- a.hat.M1[radon8.data$county]
radon8.data$m1.slope <- rep(b.hat.M1, 209)
radon8.data$m2.int <- a.hat.M2[radon8.data$county]
radon8.data$m2.slope <- rep(b.hat.M2, 209)

p1 <- ggplot(radon8.data, aes(x.jitter, y)) +
  geom_jitter(position = position_jitter(width = .05, height = 0)) +
  scale_x_continuous(breaks = c(0, 1), labels = c("0", "1")) +
  geom_abline(aes(intercept = m1.int, slope = m1.slope),
              size = 0.25,
              colour = "grey10") +
  geom_abline(aes(intercept = m2.int, slope = m2.slope), size = 0.25) +
  facet_wrap( ~county.name, ncol = 4) + theme_bw()
print(p1)
...
frame1 = data.frame(x1 = u, y1 = a.se.M2$value)
limits <-
  aes(ymax = a.hat.M2 + a.se.M2$value,
      ymin = a.hat.M2 - a.se.M2$value)
