---
title: "Reparameterization: MLE vs. Bayes"
...
about $-1.10$.  = 0.75$.  In general, the log-odds transform has the
symmetry
\[
\mathrm{logit}(\theta) 
\ = \
...
Note that when we transform the maximum likelihood estimate for log
...
(estimate minimizing expected square error) and mode (estimate
...
effects of parameterizations.  Although we can maintain maximum
likelihood estimates by ignoring Jacobians on priors (which makes
sense, given that maximum likelihood does not involve priors, but only
the likelihood), even frequentists will apply Jacobian adjustments
when dealing with transformed distributions (e.g., lognormal or
inverse gamma).
...
But, if you must do maximum (penalized) likelihood, be careful *not*
to apply the appropriate Jacobian to your penalty term.
