...
```{r comment=NA}
fit_bayes_prob <- sampling(model_prob, data=c("N", "y"), refresh=1000);
...
In this last section, we apply the Jacobian adjustment and see that
this preserves the Bayesian posterior, but alters the maximum
likelihood estimate.
...
The code in this document is licensed under the [new BSD (3-clause)](https://opensource.org/licenses/BSD-3-Clause)
license, and the text is licensed under [CC-BY 4.0](https://creativecommons.org/licenses/by/4.0/).