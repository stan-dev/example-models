...
date: "24 April 2016 (revised 15 April 2019)"
output:
  html_document:
    theme: readable
---
...
The reason the posterior mean is greater than 0.5 is that the
posterior is skewed to the right rather than being symmetric.  In the
case of this simple Bernoulli example with uniform prior, the
posterior is available in closed form (because it is conjugate), with
form
\[
...
It's also evident from the diagnostic warning about divergent
transitions that there are computational problems.  Divergent
transitions arise when the sampler runs into numerical errors
simulating the Hamiltonian (it only uses gradient information, which
is first order, and can require arbitrarily small step sizes in
...
\ = \
\frac{1}{\sigma} \times \mathsf{Logistic}\left( \frac{y - \mu}{\sigma} \right).
\]
The extra $1 / \sigma$ term derives from the Jacobian of the transform
\[
f(y) = (y - \mu) / sigma.
\]
We'll leave it as an exercise for the reader to work out the algebra
and show this is the same as the [definition in the
...
Thanks to Stijn de Waele and Jean-Gabriel Young for corrections and
clarifications.
