...
which I ran in Colab and then saved to the [GitHub repo for this report](https://github.com/stan-dev/example-models/blob/case-study/stan-cloud/knitr/cloud-compute-2020/show_VM_specs.ipynb).
This is an [IPython notebook](https://jakevdp.github.io/PythonDataScienceHandbook/01.05-ipython-and-shell-commands.html),
therefore exclamation point symbol '!' executes a shell command.

```{r figColabStats, echo=FALSE, out.width = "75%", fig.cap="Colab notebook for Colab VM stats"}
library(knitr)
...
### A few words on security and authorization

Notice the "open in colab" button - security, authorization
...
Here is the listing of the contents of the resulting `.Rmd` file:
...
You can either create a new notebook via Google Drive or upload an existing notebook.
Because you can only create new Python notebooks from Google Drive,
If you want to share a Jupyter R notebook with the world, you will need to author the notebook
locally and then upload it.

```{r figGgoogle, echo=FALSE, out.width = "75%", fig.cap="Uploads to Google Drive"}
library(knitr)
