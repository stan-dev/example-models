---
title: "Jupyter notebooks for Stan using CmdStanPy and CmdStanR"
author: "Mitzi Morris"
output:
  html_document:
    toc: true
...
is giving the entire audience quick and easy access
to the notebook via a web browser.
This report explores these challenges and
provides example notebooks for R and Python.

This report also introduces two new lightweight interfaces to Stan:
...
These interfaces have the following goals:
...
The example notebooks will use these interfaces to run Stan.
...
not have the necessary permissions to install software on their computer
or it might not have enough memory or a powerful enough CPU to run Stan.
...
But you can inspect the virtual machine instance that a notebook
is running on by executing system commands from within the notebook.
...
With Google Drive, you can create new Python notebooks or upload existing notebooks from your local machine.
...
In a talk, class, or demo situation, you don't want to spend precious introduction time waiting
...
and "CmdStanPy Example Notebook.ipynb".
