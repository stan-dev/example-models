...
To avoid this, I created a CmdStan binary for Colab by installing CmdStan on
a Colab instance and then making a tar-gzip archive of the resulting compiled executable.
Because Google is currently offering limited free storage on the Google Cloud Platform,
these CmdStan binaries are stored in a Google storage bucket in the cloud;
GitHub would be another way to host them.
...
Alternatively, you could use this [this empty R notebook](https://github.com/stan-dev/example-models/blob/case-study/stan-cloud/knitr/cloud-compute-2020/template_IR_notebook.ipynb), but of course, if you want an R notebook that runs Stan, use the example CmdStanR notebook from the previous section.
