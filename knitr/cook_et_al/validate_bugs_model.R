...
# validate BUGS model by replication
#-----------------------------------------------------------------------
library(R2OpenBUGS);
library(rstan)   
...
#  target_model name of BUGS model
...
get_bugs_fit_quantiles <- function(draw, param_names, target_model, target_data,
  ...
  fit_bugs <- bugs(target_data, inits, param_names,
       n.iter=num_iters,
       n.chains=num_chains,
       n.burnin=num_iters/2,
       n.thin=1,
       model.file=target_model,
       digits=7,
  ...
  target_matrix = fit_bugs$sims.matrix;
...
target_data = list(N, E, y, x, data$num, data$adj, data$weights);
names(target_data) = c("N", "E", "y", "x", "num", "adj", "weights");
...
# do one test
quantiles = get_bugs_fit_quantiles(draw, param_names, target_model, target_data, num_chains, num_iters);
