...
```{r}
N <- 100
data_0 <- list(N=N, a=2, b=3, sigma=5, x=runif(N, 0, 10))
```
We then run the Stan model and create the fake data:
```{r, warning=FALSE, results=FALSE}
fake_data <- stan("fake-data.stan", chains=1, iter=1, algorithm="Fixed_param", data=data_0)
```
...
```{r, results=FALSE}
y <- unlist(extract(fake_data, "y"))
data <- as.list(c(data_0, y))
