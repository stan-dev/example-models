...
theta_df = data.frame(Country = cnt_names[r_map$Cntry], ISO3c = cnt_iso[r_map$Cntry],
                      Year = r_map$Yr + year0, SupDem = theta_pe, 
                      SupDem_u95 = theta_u95, SupDem_l95 = theta_l95, 
                      SupDem_sd = theta_pe_sd)

# plot theta by country
iso_plot = sort(unique(theta_df$ISO3c))
...
for (i in 1:n_cntrys) {
  ...
  lines(x = theta_df[theta_df$ISO3c == iso_plot[i], "Year"],
        y = theta_df[theta_df$ISO3c == iso_plot[i], "SupDem"],
        lwd = 1.5, col = rgb(0, 0.4, 0, 1))
  polygon(x = c(theta_df[theta_df$ISO3c == iso_plot[i], "Year"],
              rev(theta_df[theta_df$ISO3c == iso_plot[i], "Year"])),
          y = c(theta_df[theta_df$ISO3c == iso_plot[i], "SupDem_u95"],
              rev(theta_df[theta_df$ISO3c == iso_plot[i], "SupDem_l95"])),
          col = rgb(0, 0.4, 0, 0.4), border = NA)
  text(1988, 3.5, iso_plot[i], adj = 0, cex = 0.65)
