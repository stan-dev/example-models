...
The off-diagonals are  $-{\tau}c_{ij}$
and the value of the $i^{th}$ diagonal element is $\sum_{j} c_{ij}$.
...
The individual spatial RV $w_j$ for region $n_j$ with neighbors $N(i)$
...
The density of the RV $\mathbf{w}$ is:

$$ f (\mathbf{w} \vert \Theta) \propto \tau^{(N−1)/2}  exp(−\mathbf{w}^TQ(\tau)w) $$

For subregion $n_i$, the expression $j \sim i$ denotes the set of its neighbors.
When the value of the off-diagonal elements of Q $c_{ij}$ is 1
if $n_i$ and $n_j$ are neighbors and 0 otherwise, 
the density simplifies to the "pairwise difference form", which is easily evaluated:

$$ f (\mathbf{w} \vert \Theta) \propto \tau^{(N−1)/2}
exp\left( -\,\frac{1}{2} \sum_{j \sim i}( w_i − w_j)^2 \right) $$

When the neighborhood graph contians disconnected subsets, term $tau^{(N−1)/2}$
must be changed to $tau^{(N−k)/2}$ where $k$ is the number of distinct subsets.

_NOTE:  this is only valid when all the regions in the adjacency matrix have at least 1 neighbor,
i.e., all diagonal elements are strictly positive._
...
the following fragment of a Stan program shows how to estimate
the IAR component `h` is estimated in the model block:
...
```{r glm-poisson, echo = TRUE, message = FALSE, warning = FALSE}
...
```{r rstan-poisson-estimate, echo = TRUE, message = FALSE}
...
```{r rstan-poisson-fit, echo = TRUE, message = FALSE}
...
and programming techniques shown above:
...
The following script computes these values:

```{r munge-bk, echo = TRUE, message = FALSE, warning = FALSE}
library(maptools);  
library(spdep);
...
y = observed_all_2001[all_tractIDs %in% bk_tractIDs];
x = expected_all_2001[all_tractIDs %in% bk_tractIDs];
D_W = card(nb_bk);  
...
```{r fit-poisson-car, echo = TRUE, message = FALSE}
...
```{r fit-poisson-car-v2, echo = TRUE, message = FALSE}
...
```{r fit-poisson-car-re, echo = TRUE, message = FALSE}
