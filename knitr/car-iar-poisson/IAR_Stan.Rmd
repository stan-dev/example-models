...
census tract id, the count of observed accidents per tract
and the population-adjusted rate of expected accidents per tract.
...
First we plot the observed data and the population-adjusted exposure:
...
# plot observerd accidents
bk_observed = observed_all_2001[bk_tracts]
df1 = data.frame(bk_tractIDs,bk_observed)
p1 = left_join(bkfort,df1,by=c("id" = "bk_tractIDs"))
ggplot() + geom_polygon(data=p1, aes(x=long, y=lat, group=group,fill=bk_observed)) + coord_map() + coord_fixed()  + scale_fill_gradientn(colors=blues9, limits=c(0,50))


# plot population-adjusted-rate for brooklyn
bk_expected = expected_all_2001[bk_tracts]
df2 = data.frame(bk_tractIDs,bk_expected)
p2 = left_join(bkfort,df2,by=c("id" = "bk_tractIDs"))
ggplot() + geom_polygon(data=p2, aes(x=long, y=lat, group=group, fill=bk_expected)) + coord_map() + coord_fixed()  + scale_fill_gradientn(colors=blues9, limits=c(0,50))
```
...
mean_spatial_smoothed_exposure <- apply(mu_samples,2,mean)
df3 = data.frame(bk_tractIDs,mean_spatial_smoothed_exposure)
p3 = left_join(bkfort,df3,by=c("id" = "bk_tractIDs"))
ggplot() + geom_polygon(data=p3, aes(x=long, y=lat, group=group, fill=mean_spatial_smoothed_exposure)) + coord_map() + coord_fixed() + scale_fill_gradientn(colors=blues9, limits=c(0,50))
```
