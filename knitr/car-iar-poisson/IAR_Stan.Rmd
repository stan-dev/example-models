...
- $\phi_i$ is an IAR spatial component.

The pairwise difference formulation of the IAR spatial component $\phi$ is non-centered,
thus models with include both the IAR spatial effects component are non-identifiable.
...
Given the above specification of the IAR model, the information needed to compute the
spatial variance component of the model is:
...
in any given row or column is 4 or 8, thus the matrix quickly becomes very sparse.
Representing a sparse matrix as a full matrix is computationally expensive both
...
We have written an R program
[mungeCARdata4stan.R](https://github.com/stan-dev/example-models/tree/master/knitr/car-iar-poisson/mungeCARdata4stan.R)
which defines a function of the same name that takes the
two inputs used for the WinBUGS `car.normal` function
and returns a list containing the input data objects `N`, `N_edges`, `node1`, and `node2`.
The conditional on the innermost loop does the equivalent work of restricting
the edges recorded in the parallel arrays `node1` and `node2` to entries in the
...
The values for the priors are the same as in the Carlin model.
```
...
To match the Carlin model, where covariate `aff` is divided by 10, we multiply the `x` covariate by 0.1,
...
As a comparison, we include an R script to run the WinBUGS model in OpenBUGS on the same data:
...
We have not yet determined whether or not this is due to differences in the specification of the model
or differences in the samplers. 

We note that this model is extremely sensitive to the priors on `tau_theta` and `tau_phi`.
In the Stan model, when these priors are set to normal(0, 1) priors, the fitted model agrees more closely
with the WinBUGS model.
In particular, it greatly lowers the value of `tau_phi`.
...
[bym_pred_only_norm_priors.stan](https://github.com/stan-dev/example-models/tree/master/knitr/car-iar-poisson/bym_pred_only_norm_priors.stan).
This program implements the BYM model for univariate data without an offset term, with normal priors on `tau_theta` and `tau_sigma`.
