...
A commonly-used model in biostatistical and epidemiology used for count data
is a lognormal Poisson model proposed in Besag York Mollié 1991, which
...
Banerjee Carlin and Gelfand 2003, section 5.4, provides a good discussion
...
Their formulation of the Besag York Mollié model is:
...
_pairwise difference prior_ which is identified up to
...
and specifying a separate intercept term with a location invariant Uniform($-\infty$, $+\infty$)
prior is equivalent to the unconstrained parameterization with no separate intercept. 
WinBUGS 1.4 includes a distribution called `dflat()` which corresponds to an improper (flat)
prior on the whole real line, which must always be used as the prior for the intercept term
in a model which includes an IAR component, and the WinBUGS and GeoBUGS packages function
`CAR.normal` perform the recentering of $\phi$.
Because Stan works on the unconstrained parameters scale, we can drop
the group-level intercept `beta_1` from the model, and then 
construct the intercept term in the generated quantities block.


Another challenge for this model is determining how much extra-Poisson variability
...
All commands should be run from the GitHub repo
`stan-dev/example-models`  directory `knitr/car-iar-poisson`.
...
The Stan program `scotland_bym.stan`, included in the GitHub repo,
...
```{r loadpkgs, comment = NA, message = FALSE, warning = FALSE}
library(gpclib);
library(maptools);  
library(spdep);
gpclibPermit()
...
```{r loaddata, comment = NA, message = FALSE, warning = FALSE}
...
```{r shpdata, comment = NA, message = FALSE, warning = FALSE}
...
```{r fitscotbym, comment = NA, message = FALSE, warning = FALSE, eval=FALSE}
...
```{r setupCARBays, comment = NA, message = FALSE, warning = FALSE}
library(CARBayes);
...
The Stan program is in the file `brooklyn_bym.stan`,
which is a variant of `scotland_bym.stan`, differing only in that
since the population has already been rate-adjusted and scaled,
there's no need for an offset term, or for the transformed data block.

In order to run this on the NYC study data, we restrict our attention to Brooklyn tracts only,
which meets the criteria that all regions have at least one neighbor.
