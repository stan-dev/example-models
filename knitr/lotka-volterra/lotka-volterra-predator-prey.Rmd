...
Howard (2009, Figure 2.10) provides the following approximate point estimates for the model parameters based on the data.
$$
\hat{\alpha} = 0.55, \ \
\hat{\beta} = 0.028, \\
\hat{\gamma} = 0.84, \ \
\hat{\delta} = 0.026
$$
Howard then plugs in these point estimates and derives the most likely populations $z$ (including the initial population $z_0$).

Rather than plugging in point estimates to get point predictions, we will adjust for the two forms of uncertainty inherent in our model.  First, there is estimation uncertainty, which we characterize with the posterior density $p(\alpha, \beta, \gamma, \delta, z_0, \sigma \mid y)$.  The second form of uncertainty is the observation error and unexplained variation, which are both rolled into a single sampling distribution, $\log y_n \sim \mathsf{Normal}(\log z_n, \sigma)$.  As in the Stan implementation, $z_n$ is the solution to the differential equation conditioned on the parameters $\alpha, \beta, \gamma, \delta$ and initial state $z_0$.  Altogether, we will be simulating new $y$ values, which we write as $y^{\mathrm{sim}}$, according to the posterior predictive distribution,

$$
p(y^{\mathrm{sim}} | y)
\ = \
\int p(y^{\mathrm{sim}} | \theta) \ p(\theta | y) \ \mathrm{d}\theta.
$$
...
\hat{y}^{\mathrm{sim}}
& = &
\mathbb{E}[y^{\mathrm{sim}} | y]
\\[8pt]
& = &
\int y^{\mathrm{sim}} \, p(y^{\mathrm{sim}} | y) \ \mathrm{d}y^{\mathrm{sim}}
\\[8pt]
& = &
\int y^{\mathrm{sim}} \, p(y^{\mathrm{sim}} | \theta) \, p(\theta | y) \ \mathrm{d}y^{\mathrm{sim}} \ \mathrm{d}\theta
\\[8pt]
& \approx &
\frac{1}{M} \sum_{m=1}^M y^{\mathrm{sim}(m)}
\end{eqnarray}
$$
As with other posterior expectations, the Bayesian point estimate is given by a simple average over simulated values, where $y^{\mathrm{sim}(m)}$ is just the result of simulating the value of $y^{\mathrm{sim}}$ according to the generative model based on parameter draw $\theta^{(m)}$.

The posterior predictive estimates of the dynamics is shown below, along with the raw data on number of pelts collected.
...
population_plot2
```
In the lynx and hare data, the periodicity appears to be somewhere in the neighborhood of ten to twelve years.
