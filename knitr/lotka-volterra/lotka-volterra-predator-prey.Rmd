...
```{r echo=FALSE, fig.margin=TRUE, fig.cap="Plot of the number of pelts collected for lynx versus hares evolving from 1900 to 1920.  This plot is similar to that of the dynamics of a spring in phase space (position, momentum)."}
...
If the initial values are strictly positive, i.e., $u(0) > 0$ and $v(0) > 0$, then the values at other times will be positive.  Thus
...
1. If both population sizes are initially positive, the populations will oscillate in a fixed pattern indefinitely, remaining positive sized.
...
Bayes's rule gives us a general solution to the inverse problem in the form of a posterior probability function $p(\theta | y)$.^[Bayes's rule for parameters $\theta$ and observed data $y$ is $$ \begin{array}{rcl} p(\theta\,|\, y) & = & \displaystyle \frac{p(\theta, y)}{p(y)} \\[8pt] & = & \displaystyle \frac{p(y | \theta) \, p(\theta)}{p(y)} \\[8pt] & = & \displaystyle \frac{p(y | \theta) \, p(\theta)}{\int_{\Theta} p(y | \theta) \, p(\theta) \, \mathrm{d}\theta} \\[8pt] & \propto & p(y | \theta) \, p(\theta). \end{array} $$]
...
1.  *Missing data*.  Suppose that several of the measurements are missing.  Write a Stan program that uses only the observed measurements.  How robust is the fit to missing a few data points?  What happens to the computation and posterior inferences when most of the data is missing?  How can the missing data points be imputed?
...
1. *Model misspecification.*  Andrew Gelman's [folk theorem](http://andrewgelman.com/2008/05/13/the_folk_theore/) says that if there's something wrong with your computation, maybe there's something wrong with your model.  Swap the coding of the lynx and hare in the input so that the predator is modeled as prey and vice-versa.  What happens to the model fit time?  R-hats?  How well does it fit the data?
...
* Stan Development Team (2017) *Stan Modeling Language Users Guide and Reference Manual*, Version 2.17.0, [http://mc-stan.org](http://mc-stan.org).
...
All of the source for the text and code is available on GitHub from 
