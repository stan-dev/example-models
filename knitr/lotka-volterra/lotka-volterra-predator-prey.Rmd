...
As can be seen from the diagram, the population dynamics orbit in a stable pattern.


## The Lotka-Volterra Equations

The hare population intrinsically grows, but larger number of lynxes cause it to drop.  The lynx population intrinsically shrinks, but larger number of hares cause it to rise.  Together, these dynamics lead to a cycle of rising and falling populations.  With a low lynx population the hare population grows.  As the hare population grows, the lynx population grows.  Eventually the lynx population is such as to cut down on the hare population, and the lynx population itself drops.

The Lotka-Volterra equations (Volterra 1926, 1927; Lotka 1925) use a pair of first-order differential equations to describe the population dynamics of a pair of species, one predator and one prey  Suppose that $u(t) \geq 0$ is the population size of the prey species at time $t$ and $v(t) \geq 0$ is the population size of the predator species.  Volterra modeled the temporal dynamics of the two species (i.e., population sizes over times) in terms of four parameters, $\alpha, \beta, \gamma, \delta > 0$, as
$$
\begin{eqnarray}
...
The Lotka-Volterra model is deterministic.  Given the system parameters and the initial conditions, the population dynamics are fully determined.  We will consider two sources of error.
...
The second source of error is noisy measurements of the population.  We cannot measure the population directly, so instead make do with noisy measurements, such as the number of pelts collected.  These measurements may be supplemented with output of other measurements, such as mark-recapture data.

We will specify a statistical model that allows us to infer the parameters of the model and predict future population dynamics based on noisy measurements and a model that does not explain all of the observed variation in the data.
...
The data $y_i$ consists of measurements of the prey $y_{i, 1}$ and predator $y_{i, 2}$ populations at times $t_i$.  The Lotka-Volterra equations will replace the determinsitic parts of the equations.

The true population sizes at time $t = 0$ is unknown---we only have a measurement $y0$ for it.  The true population initial population sizes at time $t = 0$ will be represented by a parameter $z0$, so that
...
Next, let $z_1, \ldots, z_N$ be the solutions to the Lotka-Volterra equations at times $t_1, \ldots, t_N$ given initial conditions $z(t = 0) = z0$.  Each $z_n$ is a pair of prey and predator population sizes,
$$
z_{n, 1} = u(t_n)
\ \ \ \mathrm{and} \ \ \
z_{n, 2} = v(t_n)
$$
The $z_n$ are deterministic functions of $z0$ and the system parameters $\alpha, \beta, \gamma, \delta$, and ae thus not parameters of the model.
...
\end{eqnarray}
$$
where the $z_n$ are the solutions to the Lotka-Volterra equations at times $t_1, \ldots, t_N$ given initial population $z0$.  The prey and predator populations have error scales $\sigma_1$ and $\sigma_2$.
...
ts <- 1:N
y0 <- c(lynx_hare_df$Hare[1], lynx_hare_df$Lynx[1])
...
Finally, the compiled model and data are used for sampling.  Stan's default settings are sufficient for to fit this data set to this model.
...
This posterior predictive check shows that the model fit is consistent with the data.
