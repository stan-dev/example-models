...
where $\Theta$ is the support of the parameters $\theta$.  The basic
form is important here, as it's an expectation over the posterior,
which always has the form $\int_{\Theta} \ \ldots \  p(\theta \, | \,
y) \ \mathrm{d}\theta$, which amounts to averaging the $\ldots$ over
the posterior.

We can also define the predictive log density as an expectation in the
same way as
...
simulations $y^{\mathrm{rep} \ (m)}$ according to $p(y^{\mathrm{new}}
\, | \, \theta^{(m)})$.  This latter computation can usually be done
efficiently (both computationally and statistically) by means of
forward simulation from the sampling distribution.
...
as the sum of the log densities of the new observations.
...
($\mathsf{Logistic}(0,1)$) distribution.
...
and 80% intervals compare for $\kappa$ and $\phi$?
...
1996 American League season of Major League Baseball.  The data was
