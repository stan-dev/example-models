---
title: "A/B Testing with Multi-Armed Contextual Bandits in Stan"
...
for (n in 1:MAX_N) {
  ...
  fit <- sampling(mod, data = data, chains = 3, iter = 1000, refresh=0,
                  init = 1, control = list(stepsize=0.1, adapt_delta=0.95))
  p_best[n, ] <-
...
The Thompson sampling probability is given by implementing our above approach.

```{r}
fit_bernoulli_bandit <- function(y, z, K) {
  data <- list(K = K, N = length(y), y = y, z = z)
  control <- list(stepsize = 0.1, adapt_delta = 0.98)
  sampling("bernoulli-bandits.stan", data = data, 
           init = 1, control = control, 
           chains = 4, iter = 1000, refresh = 0)
}

posterior_expectation <- function(fit, param) {
  posterior_summary <- summary(posterior, pars = param, probs = c())
  posterior_summary$summary[ , "mean"]
}

bernoulli_prob_matching_policy <- function(y, z, K) {
  posterior <- fit_bernoulli_bandit(y, z, K)
  p_best <- posterior_expectation(posterior, "is_best")
  sample(K, 1, replace = TRUE, p_best)
}  
```

Written modularly this way, with only the probability matching part of the code to compute p_best and then sample from it, the code is much easier to understand.   In the previous version, several outside concerns were mixed togehter such as saving results for later and pulling results out of larger arrays for reuse.  In the next section, we'll see where all that code is going to wind up.
...
```{r}
bandits <- c(bernoulli_bandit_factory(0.5),
             bernoulli_bandit_factory(0.4))
policy <- bernoulli_prob_matching_policy
N <- 20
yz <- sim_policy(N, policy, bandits)
yz
```

But it no longer saves the intermediate probability estimates, so it's not so easy to plot.
