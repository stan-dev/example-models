...
## Finite mixture model generative process
...
The stationary distribution may be computed by repeated multiplication of the stochastic matrix,^[Natural *powers of matrices* are defined in the base case by $\theta^0 \ = \mathrm{I}_K$ and in the inductive case by $\theta^{n + 1} = \theta^n \ \theta$.]
$$
\psi
\ = \
...
\theta^n
$$
It is more efficient to grow the sequence multiplicatively^[The difference is $\mathcal{O}(n)$ versus $\mathcal{O}(\log n)$.] by setting
$$
