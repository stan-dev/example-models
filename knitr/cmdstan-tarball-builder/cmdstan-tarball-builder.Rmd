...
portable (mac/linux/windows) cmdstan tarball from a combination of cmdstan,
stanc, stanc3, and math Github branches. The goal of this Github Action is to
make it easier to share experimental Stan work.
...
a **Run Workflow** button at the bottom of this form that will start the
calculation.
...
the tarball directly from the release page and use it and for `cmdstanr` users
to use the `install_cmdstan` command with a custom `release_url`.
...
```
install_cmdstan(release_url = "https://github.com/stan-dev/cmdstan/releases/download/v2.26.1/cmdstan-2.26.1.tar.gz")
```
