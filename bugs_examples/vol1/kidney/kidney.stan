...
data {
  ...
  int<lower=0> N_uc;
  int<lower=0> N_rc;
  array[N_uc] real<lower=0> t_uc;
  array[N_rc] real<lower=0> t_rc;
  array[N_uc] int disease_uc;
  array[N_rc] int disease_rc;
  array[N_uc] int patient_uc;
  array[N_rc] int patient_rc;
  array[N_uc] int sex_uc;
  array[N_rc] int sex_rc;
  array[N_uc] int age_uc;
  array[N_rc] int age_rc;
}
parameters {
  real alpha;
  real beta_age;
...
model {
  ...
  for (i in 1 : N_uc) {
    t_uc[i] ~ weibull(r,
                      exp(-(alpha + beta_age * age_uc[i]
                            + beta_sex * sex_uc[i]
                            + yabeta_disease[disease_uc[i]]
                            + b[patient_uc[i]])
                          / r));
  }
  for (i in 1 : N_rc) {
    1 ~ bernoulli(exp(-pow(t_rc[i]
                           / exp(-(alpha + beta_age * age_rc[i]
                                   + beta_sex * sex_rc[i]
                                   + yabeta_disease[disease_rc[i]]
                                   + b[patient_rc[i]])
                                 / r),
                           r)));
