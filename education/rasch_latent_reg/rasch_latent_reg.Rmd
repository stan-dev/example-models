...
The **Stan** program is nearly a direct translation of the notation above. The constraint placed on the item difficulties is enacted by creating `beta_free`, which contains the $I-1$ unconstrained item parameters, in the parameters block. Then in the transformed parameters block, `beta` is created to be identical to `beta_free` except for one additional element that is the constrained item difficulty. As a result of this constraint, the mean of `beta` will be zero.
