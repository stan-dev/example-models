...
functions {
  ...
  */
  real bivariate_poisson_log_lpmf(array[] int n, real log_lambda,
                                  array[] real p) {
...
transformed data {
  ...
  array[7] int<lower=0, upper=R> num_obs_site;
...
parameters {
  ...
  real<lower=0> sd_lam;
  real<lower=0> sd_p;
  array[R, T] vector<lower=-7, upper=7>[7] logit_p; // Originally `lp' in the BPA book
}
...
generated quantities {
  ...
  {
    array[R, 7] int N; // Abundance
    array[R, T, 7] real eval; // Expected values
    array[R, T, 7] real y_new; // Replicate data
    array[R, T] vector[7] p;
