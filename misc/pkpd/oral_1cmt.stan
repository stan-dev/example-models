...
#include "model_lib.stan"
  ...
  matrix pk_system(vector lref, vector Dt, vector theta) {
    ...
    return(pk_1cmt_metabolite_depot(lref, Dt, theta[1], theta[1], theta[2], 0, 0));
  }
  ...
  matrix pk_system_addl(vector lref, vector Dt, int cmt, real lamt, real tau, real n, vector theta) {
    ...
    */
    lstate_mdose <- pk_1cmt_metabolite_depot(lref_mdose, Dt - tau * n, theta[1], theta[1], theta[2], tau, n+1);
    for(s in 1:S)
